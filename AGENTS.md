...
Every guard above stops one question short. The shape guards ask whether a declared column is **present**; `_audit_index_collapse` asks whether the index is **unique**; Sites R and B ask whether the column holds **anything at all**. None asks whether what it holds is **possible**. Tanzania's `crop_production` 2020-21 reports one household harvesting **7,500,000 kg of coconuts** from a single plot — present, non-null, correctly typed, uniquely indexed, in a cell that grades `sane`, and nothing said a word.

**Site Q** (`Country._finalize_result`, beside Site B) reports a screened value that exceeds **100× the 90th percentile of its comparison cell** — the finest of `(t, u, j)` → `(t, u)` → `(t)` holding ≥ 30 rows with a strictly positive quantile. It **counts and NAMES** — household, plot, crop, unit, wave, value, reference, ratio — and **never clips, drops or NaNs**, the same contract `_screen_reported_factors` states for the reported `KgFactor`. `check_quantities` returns its input frame unchanged, by contract. Only `crop_production.Quantity` is screened today (`_SCREENED_COLUMNS` is the extension point; `food_acquired` is deliberately out).

**The 90th percentile is not a rounding of the 99th — it is the whole trick.** At the 30-row floor, a percentile at rank `q·(n−1)` is immune to the top `(1−q)·(n−1)` values, so p99 is immune to nothing. Measured on Tanzania's `(2020-21, kg, Timber)` cell (n=37): p99 = 2,632,000 — *66% of the very row under test* — p95 = 70,800 (eaten by the second outlier at 200,000), p90 = 23,088 (clean). Under a p99 reference the 4,000,000 kg Timber row #857 names does not fire at any threshold that leaves the corpus quiet.

**`K = 100` is a stated tolerance, not a discovered boundary — there is no gap here, and that is the difference from the null-read trigger.** `_NULL_FRACTION_TRIGGER` could sit "in the gap" because one existed. The ratio `Quantity / reference` has none: over 277k rows it runs p90 = 1.0, p99 = 3.7, p99.9 = 20, p99.99 = 174, max 11,029, smoothly. So 100 is *argued* — two orders of magnitude above what nine in ten comparable rows report, five times the corpus's own 99.9th percentile, and below the Timber row's 173.3 — and the sensitivity is recorded so it stays re-checkable (K=50 → 94 rows, K=100 → 49, K=300 → 21 on four countries).

**Measured firing, warm parquets, no rebuild: 85 rows of 427,551 (0.02%)** — Tanzania 14, Uganda 36, Malawi 16, Nigeria 9, Ethiopia 10; only the country being read warns. Cost 43 ms (Tanzania) to 257 ms (Uganda, the largest `crop_production`), the same order as Site B's 196 ms. **Deliberately no allowlist**, same reasoning as `_grain_strict`. Warn by default; **`LSMS_QUANTITY_STRICT=1`** makes it fatal (`QuantityImplausibleError`) — **its own lever, not `LSMS_READ_STRICT`**: a destroyed row, an empty column and an impossible value are three concerns and must ratchet separately. `quantity_reports(country=, table=)` is the accessor, twin of `null_read_reports()`; each report carries every offender, not just the dozen the warning prints.

**What it does not see.** A *repeated* sentinel: Uganda holds 3,097 rows of `Quantity == 99999` and only **9** fire, because the sentinel saturates most cells it appears in and is therefore its own 90th percentile. That is correct — a repeated sentinel is a different defect from a lone extreme, and GH #861 owns it. Site Q also cannot judge a row whose cell is too thin at every rung (silence, not a guess), and it says nothing about a value that is wrong but *plausible*.
