...
`served_na_new = 0` is the point: the rule never serves a cell as absent where `first()` found a value in the same group, so the two cells the census hand-checked both improve — Niger `household_roster` 2014-15 `('2014-15','101008','1')` serves the household head's populated row rather than the blank line that shares its broken `pid`, and Nigeria `cluster_features` 2013Q1 `13/1310/1300` keeps the GPS fix four of its five households report. What *does* change (544 rows, e.g. Kosovo `housing` 2000 on `Type`/`Electricity`) is which equally-complete row is served. **`Derivation`** is the one column with its own reducer beyond the additive measures: the `MULTI_SEP`-join of the group's sorted distinct keys on the additive branch (a sum of N rows is derived if any input was, so a single key would under-claim, and it joins the additive re-audit's reconciled list there), and the selected row's own key otherwise. The additive overlay is also **cython now** — `grouped[additive].sum(min_count=1)` and a vectorised `Derivation` union (`derivations.union_keys_by_group`) in place of Python groupby callables, which pandas pays for by `deepcopy`ing `df.attrs` **once per group**: measured 0.12 s vs 51.8 s and 0.25 s vs 12.4 s on a 400k-row/200k-group frame, byte-identical output, and the difference between GhanaLSS `food_acquired` collapsing and a 20-minute test timeout. `Wave.cluster_features` now carries `@build_transform(tables=['cluster_features'])` because Site 2 previously reached **no** table's fingerprint at all; Site 1 is tagged `tables=()`, so **every** L2-country hash moves and the corpus re-warms once.
