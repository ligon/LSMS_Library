...
**The floor nothing gates.** `min_baseline=5` is ABSOLUTE: nine reports clear it and then govern 10,107 rows with no relation between the two counts, and the strict rung has no dispersion check at all -- measured, it admits `(t,j)` cells whose own reports differ by up to 9e7x (median max/min 7.5 Uganda, 178 Malawi, 500 Ethiopia). The D5 tolerance the design argued over governs 0-0.7% of rows; this governs 35-98%. `conversion_to_kgs(baseline_max_spread=...)` is the prepared lever -- `p90/p10` at N >= 10, `max/min` below -- and **defaults to None (off) pending a chosen value**; the sweep for 3/5/10/30 is in the impl ledger. It refuses Niger's millet baseline at every candidate.
