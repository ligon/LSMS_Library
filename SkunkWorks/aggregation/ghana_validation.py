...
def _revision():
    """The LSMS_Library commit the run was made from (was a hardcoded string)."""
    import subprocess
...
def _cfe_version():
    ...
    try:
        return importlib.metadata.version('CFEDemands')
...
def run_comparison(data, specification, *, names=None, seed=20260916,
    ...
    metadata = dict(**data.profile, seed=seed, evaluation=evaluation, reserved=list(reserved),
                    ...
                    partitions=list(maps), lsms_revision=_revision(), cfe_revision=_cfe_version())
