...
def main():
    ...
    parser.add_argument('--allow-append', action='store_true',
                        help='permit rows for delivered labels absent from the country table '
                             '(post-#935 the served j is a bijection onto that table, so an '
                             'append means a wave label leaked; refused by default)')
    args = parser.parse_args()
    spec = yaml.safe_load(args.specification.read_text())
    delivered = pd.read_csv(args.inventory)['Preferred Label']
    result = publish(args.org_file, spec, delivered, args.output)
    print(result)
    if result['appended_rows'] and not args.allow_append:
        raise SystemExit(f'{result["appended_rows"]} delivered labels were absent from the '
                         'country table and have been appended; that is the pre-#935 leak. '
                         'Fix the crosswalk (or pass --allow-append if it is intended).')
