...
*Still open, same root cause, deliberately not fixed here.*
=_/enterprise_income.py= is dense in exactly the same way, and by the same
two artefacts: the [2026-06-26] coverage snapshot sums its eight wave slices
to *13 935* rows, =uganda_baseline.json= records =var/enterprise_income.parquet=
at *78 040* = 9 755 ids x 8 waves.  =_/assets.py= uses the same =stack()= /
dict / =stack()= idiom (its column axis carries the asset type rather than a
single measure, so the cross product is over =(id, asset)= pairs), but
*its density is not measured here* -- no =var/assets.parquet= entry exists in
the baseline to compare against the 116 808 rows the coverage snapshot sums
to.  Neither table has the naming defect, so neither shows up in the test
suite; both want the same rewrite, each measured on its own.

*Baseline debt.*  =tests/fixtures/uganda_baseline.json= pins content hashes,
and the new ones are not computable without microdata access.  The two
affected entries carry =null= in the fields that could not be recomputed
(see the =test_uganda_invariance.py= docstring); =python
tests/generate_baseline.py lsms_library/countries/Uganda= on a machine with
data clears them.
