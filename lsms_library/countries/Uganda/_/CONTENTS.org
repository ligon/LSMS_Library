...
*Same API call, two answers, decided by cache provenance.*  That is how
GH #772 reached the reporter as 129 160 rows: the =var/earnings.parquet= on
that machine had been written by the Makefile chain, not by the wave
concatenation.  It is also why =assets= reads clean -- nothing had triggered
