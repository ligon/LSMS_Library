...
*=enterprise_income= is still dense -- confirmed, not inferred.*  64 105 of
its 78 040 rows (*82.1 %*) are entirely null.  It has no naming defect, so
nothing in the test suite reports it.  It wants the same =pd.concat= rewrite.

*=assets= is NOT dense, and the reason is its own puzzle.*  It measures 1.00x
with zero all-null rows despite =_/assets.py= using the identical =stack()= /
dict / =stack()= idiom -- because *the served table is not that script's
output at all*.  =assets.py= writes a column named =assets= indexed
=(i, t, m)=; =var/assets.parquet= holds a column named =Value= indexed
=(t, i, j)=, which is exactly the wave parquets concatenated.  The bare
=!make= tag desugars to ={materialize: make}= (=yaml_utils._construct_make=),
identical to the =materialize: make= that =earnings= uses and that *did* run
its script -- so this is not a config difference.  Either =assets.py= is dead
code or the framework is silently serving a wave concatenation in its place.
*Recorded as an open question, deliberately not chased here*; it wants its own
issue, because whichever answer is true is worth knowing corpus-wide.
