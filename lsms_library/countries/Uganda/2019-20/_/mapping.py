...
def v(x):
    ...
    free-text parish name ``s1aq04a``, and 23 parish names recur across
    districts, fusing distinct parishes into one "cluster" that
    ``groupby().first()`` then resolved arbitrarily.

    ``district`` + ``s1aq04a`` (the source's district / parish name columns)
    makes ``v`` a real key.  ``sample`` and ``cluster_features`` declare the
    same pair, so they stay in lock-step and ``_join_v_from_sample`` propagates
    the same key to every other table.

    Returns ``None`` when either component is blank, so an incompletely-located
    household gets no cluster rather than a fabricated one.  Exactly one row in
    this wave is blank in both columns; it has no cluster today either.
    """
