...
|------+---+---+---+---|
| 2018-19 | 204,581 -> 194,173 | 171,946,368 -> 147,345,571 (-14.3%) | 0 -> 10,802 (55,371) | 44,739 (23.0%): 4,768 / 27,631 / 12,340 |
| 2021-22 | 211,365 -> 200,917 | 185,854,825 -> 182,628,314 (-1.7%) | 0 -> 10,917 (47,892) | 30,621 (15.2%): 5,264 / 15,645 / 9,712 |
...
* 2026-09-18 -- =Relationship= served a missing answer as the string 'Nan' (GH #813)

Both waves' =_/mapping.py= guarded the =s01q02= formatter with =if value:=,
and a float =nan= is *TRUTHY* -- so every unanswered relationship went through
=str(nan).title()= and was served as the literal label =Nan=.  Not a survey
vocabulary item: the string is the pipeline's, not the instrument's.

*Rows*: 15,201 of 78,732 in 2021-22, 3 of 66,119 in 2018-19 -- exactly the rows
whose =s01q02= is null in =s01_me_sen2021.dta= / =s01_me_sen2018.dta=.

*What the 15,201 are, and why it matters.*  2021-22 is a panel wave, and Survey
Solutions pre-populates the Section-1 roster grid from the 2018-19 enumeration.
On all 15,201 rows =s01qpreload_relation= is populated (15,200 of 15,201) while
every LIVE Section-1 column is null: =s01q01= (sex), =s01q03a= (DOB year),
=s01q04a= (age), =s01q00=, =s01q12=.  They are CAPI ghost rows -- people the
preload offered and the interviewer did not enrol.  The wave's own
=household_roster= hook already says so and already drops them:
...
*that filter could never fire*, because =Relationship= was guaranteed to be a
non-null string.  With the =pd.isna= guard in place it fires, and the wave
parquet holds 63,531 rows for 2021-22 instead of 78,732.

*Served data did not move.*  =_expand_kinship='s =_NA_SENTINELS= (commit
0d35af6ed, April 2026) already mapped ='Nan'= to NA, and =_finalize_result='s
=dropna(how='all')= already removed the ghost rows downstream, so
=Country('Senegal').household_roster()= is row-for-row and label-for-label
identical before and after (129,650 rows; 66,119 + 63,531; same
=(Generation, Distance, Affinity)= counts).  What the fix buys is that the
defect is gone from the cache and the country no longer leans on a framework
sentinel list to undo its own bug.

*Do not confuse this with the encoding work.*  The =.title()= call is untouched
for non-null values -- the GH #801 =_fix_relationship= repair depends on that
spelling -- and nothing here goes near the mojibake-keyed =u= / =j= crosswalks
warned about above.

*Latent elsewhere*: =Niger/2018-19/_/mapping.py= carries the identical
=if value:= formatter.  Niger's =s01q02= has no nulls, so it emits no ='Nan'=
today; left alone deliberately, so the GH #813 fix stays provably targeted.
