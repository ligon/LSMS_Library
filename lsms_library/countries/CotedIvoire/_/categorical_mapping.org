...
# and harmonize_input maps every seed label to the single `input` value
# 'Seed' -- so any two DISTINCT reported seed items that share a Preferred
# Label collide on one index tuple and the framework's groupby().first()
# silently DISCARDS one of them.  Until 2026-07-12 four labels shared a
# single 'Autre crop' catch-all bucket; two of them actually occur in the
# data ("Autres semences" 404 rows, "Semences d'autres céréales" 86 rows),
# 11 households reported BOTH, and the 9 of those that also shared a unit
# lost a row apiece.  e.g. grappe 744 / menage 12 reported "Autres semences"
# 400 kg (own-production) AND "Semences d'autres céréales" 350 kg (market-
# purchased); the 350 kg market row was silently dropped.  The two are now
# split ('Autre crop' / 'Autre céréale'), which is also what the source
# means -- one IS specifically "other cereals".  "Plants/boutures de
# tubercules" (tuber cuttings) gets 'Autre tubercule' for the same reason;
# it and the bare code '20' happen to have 0 rows today.
#
