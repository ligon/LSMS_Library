...
*Burkina Faso is nevertheless immune today*, for the reason recorded above
under GH #223 Layer 2: =u= is decoded from the *numeric* =s07bq03b= code via
=mappings: ['ehcvm_units', 'Code', 'Preferred Label']=, so the =.dta= value
*labels* never enter the key path at all.  Contrast Senegal, which declares a
bare =u: s07bq03b= and therefore keys on the label text -- see
=Senegal/_/CONTENTS.org=, where the mojibake is *load-bearing*.
