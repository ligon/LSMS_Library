...
| wave    | fields gaining Area | Area NaN before -> after | conversions refused |
|---------+---------------------+--------------------------+---------------------|
| 2011-12 |                 964 |            5,933 -> 4,969 |                   3 |
| 2013-14 |               1,152 |            2,485 -> 1,333 |                   0 |
| 2015-16 |                 135 |            1,072 ->   937 |                   0 |
| 2018-19 |                  19 |               99 ->    80 |                   0 |
| 2021-22 |                   0 |              138 ->   138 |                   0 |
| TOTAL   |               2,270 |            9,727 -> 7,457 |                   3 |
...
*Implausibility is counted, never clipped.*  A converted area larger than
the LARGEST FIELD THAT WAVE ACTUALLY MEASURED BY GPS is REFUSED (=Area=
stays NaN) and counted -- 3 rows in 2011-12 against a 9.91 ha ceiling, 0
elsewhere.  Nothing is capped: a capped area is a number nobody reported.
The ceiling is the wave's own data rather than a magic constant, and it
is deliberately generous; note the ceiling is itself soft in waves whose
largest GPS field is implausible (2015-16's is 425.7 ha).
...
paddings DISAGREE: their general helper leaves =saq02=/=saq03= unpadded
(=W5.do:195-225=; zone 1 of region 1 becomes ="011"=) while the
conversion-file block pads them to two characters (=:325-341=;
="0101"=).  Their merge matched 1,566 of 14,878 rows and changed 19 field
sizes -- their own inline counts at =:694-703=.  The raw integer triple
is the same hierarchy without the collision risk.
...
=Other= (1,098), =Senga=, =Hectare=, =Kert=, =Tilm=, =Ermija=, =Rope=,
=Medeb=.
...
above for the food-side analogue).  The corroboration that the join is
keyed RIGHT rather than merely non-empty is *shipped vs inferred*: on the
18,780 rows where both exist (the metric units) the median ratio is
*1.0000* and *0.0%* lie outside [0.5, 2], in every wave.
...
rather than averaging or =first()=-ing:
  1. The WB's own duplicate, crop 74 x unit 62 at 4.34 and 6.125 in the
     W4/W5 blob.  *Keep 4.34*, drop 6.125, for continuity with W3 -- the
     same answer EPAR reaches.  A duplicate the rule does NOT cover
     raises, so a future one is loud.
...
=region=True= returns the =(t, j, u, region)= form for a caller who has
resolved a lower-case numeric =region= onto =crop_production=.  Why not
by default, with numbers:
  ...
  - Of the rows the table can serve, the share landing on a =(j, u)=
    ...
    (2015-16), 38.3% (2018-19), 28.7% (2021-22).  Material, not
    negligible.
