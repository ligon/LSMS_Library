...
both already in REG01 -- the natural follow-up).  *Whoever does that must
decide the padding once and apply it to both*, or the household-to-cluster
join will be quietly wrong rather than loudly broken.
...
** 1990 -- 39 DVC sidecars, and NOT readable by this library

Every data file in =1990/Data/= is =.SSP= (=N00A.SSP=, =EXPEND.SSP=,
=PANEL.SSP=, ... plus =FORMAT.ZIP=).  That is the SPSS/PC+ system-file format;
=get_dataframe()= handles =.dta= / =.csv= / =.parquet= only, and =pyreadstat=
does not read =.SSP= either.  *This is an acquisition / conversion blocker,
not a config gap*: no amount of YAML makes this wave readable.  Either a
Stata/SPSS conversion has to be produced and pushed, or the wave has to be
re-acquired in a readable format.  Note also that 1990 is Lima-only (see the
universe section above), so it is the least useful of the four.
...
What 1991 still needs: a wave =_/data_info.yml=; its own code lists (the
=Relationship= / =Sex= tables in =_/categorical_mapping.org= are established
for 1994 only); a float32 re-check on whatever files it reads; and above all
*a decision on the cluster, settled from the 1991 documentation rather than by
inspection*.  =segment= has 240 distinct values over 2,308 households and is
