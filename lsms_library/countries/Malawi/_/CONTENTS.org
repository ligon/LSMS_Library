...
*Not fixed, and reported*: =ag_i02c= (S/U -- "MAKE SURE TO ASK WHETHER THE
REPORTED VALUE IS SHELLED OR UNSHELLED") is likewise unread, and
=ag_g13c= is passed to =_harvest_block= as =condition== but the parameter
is *accepted and never used*.  A shelled/unshelled mismatch between
harvest and sale is a second, smaller basis error on legumes and
groundnut; it needs a =condition= level on the grain, which is a schema
...
- Source data available in DVC ::
  ...
  - =2019-20/Data/Cross_Sectional/ihs_seasonalcropconversion_factor_2020.dta=
    ...
    factors, 2026-09-09: held, unwired; only the food file is read,
    =2019-20/_/food_acquired.py:17=)
