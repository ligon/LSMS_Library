...
1. *=crop_code= is not a code, it is a VARIETY LABEL, and it is FINER than
   ...
   to ='Rice'=, six groundnuts to ='Groundnut'=.  Collapsing them is a
   REDUCTION, so =_collapse_varieties= does it with a stated rule: *a key
   survives only when every variety row under it reports the same factor;
   where they disagree the key is DROPPED*, never averaged, never modal,
   never first.  23 of 314 seasonal keys and 8 of 92 tree keys disagree --
   worst, Rice / 50 kg Bag / unshelled / Central at 29.04 vs 49.00 (1.69x),
   and Citrus / 50 kg Bag / Southern at 38.0 vs 55.0 (1.45x).  ~92% survives.
...
| file     | rows | crop matched | unit matched | condition | region | keys kept | keys refused |
|----------+------+--------------+--------------+-----------+--------+-----------+--------------|
| seasonal |  857 |      857/857 |      703/857 |   857/857 | 857/857 |       291 |           23 |
| tree     |  153 |      153/153 |      110/153 | (no column) | 153/153 |        84 |            8 |
...
** What it buys: harvest_kg before and after
...
=inferred= / =none=.  Flagged for @ligon in
=.coder/ledger/854-malawi-shipped-factors.md= §7.

** Found while doing this, NOT fixed: Cassava is missing from 2016-17 and 2019-20
...
It is five lines in =harmonize_crop=, but it ADDS ~3,200 rows to a served
table and needs its own before/after, so it was not folded into #854.  It
also caps what the tree conversion table can reach -- Cassava is that file's
largest crop, 12 of its 153 rows.  *Do not assume Malawi grew no cassava
after 2013.*
