...
=2018-19/Data/= ships *no* price file (checked the full 52-file listing).

The =data_scheme.yml= comment is right that =ehcvm_prix= has no =grappe= key and
so cannot join the =sample().v= keyspace -- but it *does* carry =region= and
=milieu=, which is the rung at which Mali's price comparisons actually happen
(see above).  So this is a =todo= verdict candidate for
=Mali/community_prices/2021-22=, not an absence: the data is there, the config
is not.  =ehcvm_nsu_mli2021.dta= is separately valuable -- it is a per-region
unit-weight table that would replace the inferred kg factors for that wave.
