...
The next-steps above were run with data access.  The date lever is real
and is now wired as a separate crosswalk feature; the district crosswalk
is a dead end; the interviewer/team-id idea is unavailable.
...
**District crosswalk --- abandoned; drop district instead.**  The two
encodings are structurally different, not a fixable bug: community =id_02=
is a national district code (11..128+), the survey cluster =DD= is a
within-region index (1..9), so =(region, district, ward)= = *0/488* in
2020-21.  But =region= (30/30) and =ward= (105/110) DO share a codespace,
so the match keys on =(region, ward)= and breaks ward collisions by date.

**Measured unique-cluster resolution** (honest; the desk estimates above
were optimistic):

| wave    | geo-unique | +date | total unique | region fallback |
|---------+------------+-------+--------------+-----------------|
| 2019-20 |         39 |   +12 |        51/99 |              48 |
| 2020-21 |        302 |  +112 |      414/488 |              74 |

Window is wave-specific (from a sweep): 2019-20 widens well (±14d optimal,
sparse Extended Panel); 2020-21 must stay narrow (±7d --- the dense
Refresh Panel re-ambiguates when widened, exactly the gap an interviewer
id would have closed).  Every resolved =cluster= is a real =sample().v=
(51/51, 414/414), so the household join is valid.
...
- =tanzania.link_community_to_cluster(t, cm_a, hh_a, window_days)= --- the matcher;
