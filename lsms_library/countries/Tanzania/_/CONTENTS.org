...
** WAITING Fix r_hhid & UPHI matching problem for 2008-15
The current =panel_ids.py= uses =map_08_15()= for the 2008--15 waves,
which maps every split-off household back to the parent's canonical
UPHI-based ID.  After =id_walk= this collapses split-offs and
originals into the same =(i, t, j)= index, producing ~33k duplicate
rows in the aggregated =food_acquired= (2,094 affected household-wave
pairs, concentrated in 2010-11 and 2012-13).  The proposed solution
below---appending split suffixes to preserve uniqueness---has not yet
been implemented.
