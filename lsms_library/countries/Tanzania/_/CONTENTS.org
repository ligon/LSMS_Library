...
*** =new_harmonize_units= TRUNCATES every quantity to an integer (2026-09-08)

Found during the five-source price study (=slurm_logs/price_sources/Tanzania/=).
=tanzania.new_harmonize_units= scales each quantity by its unit factor with

: df[pair['quant'][i]] = df[pair['quant'][i]].astype(np.int64) * df[pair['unit'][i]]

The =astype(np.int64)= is a *floor*, applied to the quantity as REPORTED, before
the unit factor multiplies it.  The cast is not incidental --- it is what makes
the =Piece: 'p'= trick work (=int(3) * 'p' == 'ppp'=, later counted back to 3) ---
so this is not a one-line fix.  Two distinct consequences, both measured on the
source =.dta= (purchased rows with a positive reported quantity):
...
The inflation is not small where it bites: the median inflated row is a
quantity of the form =N.5= floored to =N=, i.e. a unit value overstated by
*1.5x* (p90 1.50, max 1.90).  The recent waves are the worst affected because
...
So ~1.4% of all purchased rows corpus-wide carry a ~50% overstated unit value,
and 852 rows are dropped outright.  This is *reported, not fixed*: it changes
=food_acquired.Quantity= library-wide and the =Piece= sentinel depends on the
cast.  It also explains the residual 97.0% / 98.1% (not 100%) non-null
=Quantity= on purchased rows in the two recent waves.
