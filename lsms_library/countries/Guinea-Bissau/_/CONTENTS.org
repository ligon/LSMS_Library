...
=Country('Guinea-Bissau').derivation_inputs(key, wave=...)=.
...
|------+---+---+---+---|
| 2018-19 | 104,493 -> 90,381 | 91,353,380 -> 57,552,943 (-37.0%) | 0 -> 13,818 (55,995) | 13,226 (14.6%): 3,314 / 6,218 / 3,694 |
...
- 2018-19: A (gross =s07bq03a=) 60,051,736, B (net of own production) 59,225,643, *C (served)* 57,552,943 FCFA; raw =s07bq08= was 91,353,380.
...
is settled by =ehcvm_conso=, not by the size of the number.
  ...
  Validated against =ehcvm_conso_gnb2018.dta=: on the 35 rows where the join,
  the unit match and the candidate difference all hold, the served rule x 365/7
  reproduces =depan(Achat)= within 1% for *100%* of rows -- against *48.6%* for
  B and *0.0%* for A -- and =s07bq05 * uv= reproduces =depan(Don)= for *100%*
  of 149 rows, which is why the gift value is checked and NOT served.  (Togo
  2018 the same way: 99.5% / 13.1% / 0.0%, Don 99.7% of 1,255.)
