...
safe here, and the reason is worth recording because the framework now
ENFORCES it (a future data re-release that breaks the assumption will raise
=DuplicateIndexError=, not silently pick a value):
