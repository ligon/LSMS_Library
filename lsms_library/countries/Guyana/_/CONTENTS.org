...
de-fusion, the cluster *values*, and a *cold-build* no-collapse-warning check).
A warm-cache check would pass vacuously — the collapse is baked into the cache,
so the warning never fires twice.  Prose here is documentation; the test is the
enforcement.

*Two ways that enforcement can go quietly blind, both hit here.*  (1) A warm
cache: covered by clearing the parquets and setting =LSMS_NO_CACHE=.  (2)
Matching the warning by its *message text*: the cold-build check originally
grepped for ="duplicate tuple"=, the GH #323 audit work later reworded the
message, and the check then passed against the *broken* config while seven
=GrainCollapseWarning=s were being emitted in the same run.  It now matches the
=GrainCollapseWarning= **category**.  Match on the class, never on the prose.

*Re-measured 2026-08-20* when this branch was merged up to =development=.
Three figures in the original write-up were wrong and are corrected above: the
...
- ¶29 "There were 2,610 EDs in the country, of which 616 were selected to the
  sample for the HIES" ↔ =WEIGHT.dta= has exactly *616* rows, one per ED.  That
  is the frame, and it is why joining it as a spine admits 488 EDs that hold no
  surveyed household.
...
- *=assets= sums.*  =_/assets.py= collapses per-acquisition detail rows with
  ...
  =DRBLS.dta= really does carry several rows per household: 115 of its 1,616
  households have 2-5 rows (317 rows in all), sharing one =id_nmbr= and one
  =newid= but differing in the per-block =yri*= year-of-acquisition columns
  (household =337-253-4=, for instance, reports block 07 acquired in 1991,
  1989, 1975, 1985 and 1992).

  *OPEN, and NOT a #323 question:* whether the per-block quantity is an
  INCREMENT (sum it, as we do) or the household's total holding RESTATED at
  each acquisition (in which case summing over-counts).  The data does not
  settle it — of the 658 =(household, item)= blocks appearing in more than one
  row, *372 carry an identical quantity in every row* (reads as restated) and
  *286 differ* (reads as incremental).  The item labels are worded as stocks
  ("NO. OWNED - ..."), which leans toward restated, but the questionnaire that
  would decide it, =GUY01.pdf=, is an image-only scan and no OCR tool was
  available (see the gaps table below).  Recorded rather than guessed.  It
  affects ~7% of asset households and is *identical before and after* the #323
  fix — that fix changed only which rows fall in each group, not the reducer.
- *"Makefile not found ... Unable to generate required data" is EXPECTED here,
  and is not a failure.*  Guyana declares two =materialize: make= tables
  (=assets=, =interview_date=) but ships *no* =_/Makefile=.
  =Country.run_make_target= warns and returns an empty frame when the country
  Makefile is missing; the dynamic script dispatch then builds both tables
  anyway — =_/assets.py= at the country level and =1992/_/interview_date.py= at
  the wave level.  Observed on every cold build, on =development= as well as
  after the GH #323 fix, with the correct row counts delivered both times
  (=interview_date= 1,502 before / 1,807 after).  Do not chase it as a
  regression; if it is ever worth silencing, that is a framework question, not
  a Guyana one.
