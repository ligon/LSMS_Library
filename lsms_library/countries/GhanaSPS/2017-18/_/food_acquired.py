#!/usr/bin/env python
"""GhanaSPS 2017-18 food_acquired -> canonical (t, i, j, u, s).

Source: 2017-18/Data/11a_foodconsumption_prod_purch.dta, one row per
(household, food item):
  - FPrimary   -> i  (household)
  - foodname   -> j  (food item text; harmonized via
                     categorical_mapping.org harmonize_food, '2017-18')
  - unitname   -> u  (clean text unit; harmonized via units.org
                      'harmonizedunit' '2017-18' col)
...
df = get_dataframe('../Data/11a_foodconsumption_prod_purch.dta', convert_categoricals=False)

df = df.rename({'FPrimary': 'i', 'foodname': 'j', 'unitname': 'u'}, axis=1)
df['i'] = df['i'].astype(str)
df['j'] = df['j'].astype(str).str.strip()
df['u'] = df['u'].astype(str).str.strip().replace({'': pd.NA, 'nan': pd.NA})

# Harmonize food item text -> Preferred Label via the '2017-18' column of
...
food_items = food_items[['2017-18', 'Preferred Label']].apply(lambda s: s.astype(str).str.strip())
food_items = food_items[~food_items['2017-18'].isin(['', '---', 'nan'])]
food_items = food_items[~food_items['Preferred Label'].isin(['', '---', 'nan'])]
item_map = dict(zip(food_items['2017-18'], food_items['Preferred Label']))
df['j'] = df['j'].replace(item_map)

# Harmonize native unit text -> Preferred Label via units.org '2017-18' col.
harmonized = df_from_orgfile('../../_/units.org', name='harmonizedunit', encoding='ISO-8859-1')
harmonized = harmonized[['2017-18', 'Preferred Label']].apply(lambda s: s.astype(str).str.strip())
harmonized = harmonized[~harmonized['2017-18'].isin(['', '---', 'nan'])]
unit_map = dict(zip(harmonized['2017-18'], harmonized['Preferred Label']))
