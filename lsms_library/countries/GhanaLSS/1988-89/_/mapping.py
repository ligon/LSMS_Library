...
def sample(df):
    ...
    # GLSS2 ships NO weight column of any kind -- a sweep of every
    ...
    # value.  Evidence tier: (b) IMPLIED ONLY.
    #
    # GLSS2 is the WEAKER case, and that is why the caveats matter.  GSS never
    # states self-weighting for round 2; it follows only from round 2 using the
    # same master sample and the same fixed per-workload take.  GSS also never
    # treats the re-interview attrition: GLSS2 is a ROTATING PANEL -- half the
    # GLSS1 workloads were retained for re-interview, the other half drawn fresh
    # from replicate 6 -- and no adjustment for differential attrition in the
    # retained half is documented anywhere.  A weight of 1.0 therefore assumes
    # attrition was ignorable, which nobody has shown.
    #
