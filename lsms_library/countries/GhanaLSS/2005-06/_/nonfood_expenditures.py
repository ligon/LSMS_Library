#!/usr/bin/env python
"""GhanaLSS 2005-06 nonfood_expenditures -- Section 9A, item-level, long.
...
*** THIS WAVE HAS NO MISSING-VALUE SENTINEL, AND A THRESHOLD HERE DELETES
REAL DATA. ***  GLSS5 is denominated in OLD cedi (redenominated 100:1 in July
2007), so the median reported amount is 60,000 and ordinary large purchases run
into the hundreds of millions.  The `>= 99999999` threshold this script shipped
first therefore masked 28 genuine rows carrying 4.17e10 cedi -- *** 43% of the
wave's total ***, which it silently reported as 5.43e10 instead of 9.60e10.

No member of the repdigit-9 family occurs in either column at any field width
(the tripwire in `_amount` re-checks this on every build).

WHAT IS HERE INSTEAD IS A LONG TAIL, AND IT IS SERVED, NOT CLIPPED.  The ten
largest rows carry 38.9% of the wave and the largest are implausible on their
face (8,000,008,000 and 8,000,006,000 old cedi, ~US$890k, for one item).  They
are almost certainly data-entry errors, but the library COUNTS AND NAMES
rather than dropping -- the same rule as `quantity_audit.py` -- so they are in
the table and written up in _/CONTENTS.org.  Screen them in analysis.
...
THE ITEM AXIS IS PER-WAVE.  `Code_2005-06` in the country's
...
t = '2005-06'
FN = '../Data/partb/sec9a.dta'
ID = 'hhid'
SENTINELS = frozenset()   # MEASURED; see the module docstring
WINDOW = '12 months'
