#!/usr/bin/env python
"""GhanaLSS 2012-13 nonfood_expenditures -- Section 9A, item-level, long.
...
*** THIS WAVE HAS NO MISSING-VALUE SENTINEL -- MEASURED, NOT ASSUMED. ***
Neither s9aq2 nor s9aq4 contains a single member of the repdigit-9 family at
any field width (the tripwire in `_amount` re-checks this on every build).
The column maxima are 1.08e6 and 8.0e4 GHS, and the top 0.1% of rows carry
14.9% of the wave -- an ordinary heavy tail, not a sentinel spike.

2016-17 DOES have the sentinels, and the set is declared per wave precisely so
that one wave's convention is never assumed to hold for another.
...
THE ITEM AXIS IS PER-WAVE.  `Code_2012-13` in the country's
