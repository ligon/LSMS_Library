...
*=_drop_unpriceable= now warns on every =food_prices()= call for this
country*, with =lost_fraction= 0.0097% -> *79.84%*.  The warning text was
widened in the same PR so it no longer asserts the loss is /"typically a
0-as-missing sentinel or a quantity variable mapped to the wrong survey
question"/ -- for a currency-denominated unit the NaN is *by design*.  The
warning is correct and expected; it is not a new defect.
