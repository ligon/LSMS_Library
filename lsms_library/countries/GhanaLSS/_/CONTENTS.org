...
no weight variable in the data).  Starting from GLSS4 (1998-99), a
=weight= variable is provided.  In GLSS5 (2005-06) and later, the
sample is explicitly non-self-weighting due to disproportionate
sampling of smaller regions.

Since every round is an independent cross-section, there is no
panel weight.  The single =weight= serves for both cross-sectional
and (to the extent they exist) panel analyses.
