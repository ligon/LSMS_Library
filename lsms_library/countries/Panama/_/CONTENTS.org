...
Panama had NO country-level =categorical_mapping.org= before this; GH #783
created one, and =harmonize_food= is currently its only table.

*Panama's table had no =#+name:= at all*, so it was invisible to every
name-keyed mechanism.  All three wave scripts reached it via
=df_from_orgfile(name=None)= = "the first table in the file"; all three now
pass =name='harmonize_food'= explicitly, as does =_/nutrition.py=, and the
=_/Makefile= generic rule names =categorical_mapping.org=.

*=_/nutrition.py= is NOT reachable, and was not before this change.*
=nutrition= is absent from =_/data_scheme.yml=; the Makefile's =var= target
is =food_acquired.parquet= only; the script reads
=../var/food_quantities.parquet=, which the Makefile's own comment forbids
materializing; and its =eep153_tools= import is not an installed dependency,
so it raises =ModuleNotFoundError= before reaching any of that.  GH #783
repointed its label read (measured: the same 79 =(Preferred Label, FCT ID)=
pairs the old raw =pd.read_csv(sep='|')= produced) so the reference is not
dangling.  It did not revive the script.  Contrast =CONTENTS.org='s "DONE
nutrition" / "DONE nutrition.py" entries below, which read as though the
pipeline runs; they describe intent, not a live build.

The regeneration recipe below still writes a standalone =food_items.csv=.
If it is re-run, fold its output into =categorical_mapping.org=
=harmonize_food= rather than into a standalone file, or the vocabulary
...
** The quintal is the SPANISH quintal here, and =units.json= is wrong about it

Adjudicated by @ligon, 2026-09-15, while checking whether GH #838's
=quintal=100kg= seed could reach Panama (it cannot -- see below).

=units.json= record 12 reads:

: {"Unitcode":12,"Label":"quintal","Translation":"centner","Conversion to Pounds":220.462}

*220.462 lb is 100.000 kg* -- the METRIC quintal, to six significant figures.
That is not a rounding artefact; somebody computed it.  =units.py='s
=STANDARD_POUNDS= overrides it with =100.0= lb (45.359 kg, the Spanish
quintal) and =label_to_kgs= gives the override precedence over the source.
It is the *only one of six* =STANDARD_POUNDS= entries that contradicts its
source; the other five agree within 1%.

*The override is correct and the source is wrong.*  The judgement is that
whoever compiled =units.json= took the quintal for a universal measure and
filled in the metric centner.  In Panama it is 100 libras.  Recorded here
because the override is a 2.2x correction applied silently, and nothing said
why -- the =units.py= comment states the Spanish/metric ambiguity but not
that the shipped table takes the wrong side of it.

*Not a GH #838 exposure.*  =KNOWN_METRIC= gained =quintal: 100= in #838 and
is keyed on the country-blind lower-cased label, but Panama's wave scripts
mint the =Translation= (=centner=), which is not a =KNOWN_METRIC= key.
Measured 2026-09-15 over every served =u= label: Panama serves =centner= (23
rows) and =arroba= (12), neither of which that lookup reaches; Ethiopia is the
only country in the corpus serving a literal =Quintal= (13,977 crop rows + 16
food rows), and for Ethiopia 100 kg is right.  The latent hazard is noted at
=transformations.py='s =KNOWN_METRIC= entry by commit =313a937b8=.
...
- What is needed to fix ::
  ...
  4. The existing =units.py= script should be extended to produce the
     final JSON file.
** DONE nutrition
A food conversion table, with foods keyed to the "Preferred Labels" of =categorical_mapping.org= =harmonize_food= (GH #783; was =food_items.org=).   See https://docs.google.com/spreadsheets/d/1YwfdL0LMPVl3UGo-KfyvpC7S7KR_jDL54rsVLUQOCVE/, prepared by Kaylee Vu, which indexes the FCT at http://www.incap.int/mesocaribefoods/dmdocuments/TablaCAlimentos.pdf.
...
* =Value= and =value= are TWO populations under one lowercased label (GH #770)

Panama lost *17,366 rows* from =food_prices(units='kgvalue')= and
=food_prices(units='kgprice')= (501,027 -> 483,661) when GH #770 stopped
=transformations.conversion_to_kgs= inferring a kg factor for
currency-denominated units.  That is correct.  But the 17,821 rows behind it
are *not one thing*, and the difference matters for anyone building Step 2 of
#770 (recovering the =1/price= factor from a price source).

| wave | label     |      n | =Quantity == Expenditure= | share =Quantity == 7.70= |
|------+-----------+--------+---------------------------+--------------------------|
| 1997 | =Value=   |     46 | 100%                      |                       0% |
| 2003 | =Value=   |     87 | 100%                      |                       0% |
| 2008 | =Value=   |  6,662 | 100%                      |                       0% |
| 1997 | =value=   | 11,026 | *0.06%*                   |                  *99.5%* |

** Capital-=Value= (6,795 rows) is written by the LIBRARY
=Panama/{1997,2003,2008}/_/food_acquired.py:169-175= applies the value-only
convention (food-acquired skill / GhanaLSS): /"a purchase with a monetary
value but no physical quantity/unit is carried as u='Value' with
Quantity = Expenditure"/.  These are LCU-only goods and excluding them from
the kg inference is exactly right.

** Lowercase-=value= (11,026 rows, 1997 only) is a SURVEY UNIT CODE
=Panama/_/units.json= carries =Unitcode 77, Label 'valor', Translation
'value', Conversion to Pounds null=, and =1997/_/food_acquired.py= decodes
=ga106b= through the =Translation= column -- so the enumerator's own
"measured in value" code arrives as the *lowercase* string =value=.  It is
also currency-denominated, so excluding it is right too.

*But the stated rationale is FALSE for these 11,026 rows.*  GH #770 argues
the factor exists and equals =1/price= (Value = Quantity x Price).  That
holds for the capital-=Value= population, where =Quantity= genuinely is the
expenditure.  It does *not* hold here: *99.5% of these rows carry
=Quantity == 7.70=*, the undecoded sentinel of *GH #777* -- their =Quantity=
is not a value at all, it is a parse artefact.  A Step 2 that recovers
=Quantity / price= would silently manufacture =7.70/price= kilograms for
eleven thousand rows.  *Do not build Step 2 for Panama without resolving
#777 first.*

** The two spellings got DIFFERENT factors, and the winner was luck
Before #770, =conversion_to_kgs= returned BOTH ='Value' -> 0.34891692= and
='value' -> 2.34053472= -- a *6.7x* difference -- because it groups on the
RAW label.  =_get_kg_factors= then lower-cases and keeps whichever it merges
first, so a single arbitrary key decided which of two populations' factor the
whole country used.  The PR reported only the =_get_kg_factors= view ("86 ->
85 keys, exactly ={'value'}="), which is true and hides this.

Both spellings are now excluded, so Panama's instance is gone.  *The class is
not*: swept over the 19 cached =food_acquired= tables, *11 live case-variant
collisions remain in 3 countries* (Burkina Faso 7, Malawi 2, Mali 2 --
Burkina Faso's =Calebasse= 0.35 vs =calebasse= 1.432 is 4.1x).  Those now
emit =transformations.UnitLabelCollisionWarning= instead of resolving
silently; the fix for each is to canonicalise the spelling in that country's
own unit vocabulary.

** Why Panama is unusually exposed to ALL of this
Two idiosyncrasies already recorded above compound here:

1. *No =conversion_to_kgs.json= has ever been created* (GH #117, "TODO
   conversion_to_kgs.json" above).  =units.json= ships "Conversion to
   Pounds" as =null= for most survey-specific units, so *every* kg factor
   Panama has is inferred from price ratios -- the same "everything is
   inferred" condition that made GhanaLSS the worst case.  There was no
   shipped table for =value= to be absent from, and therefore nothing to
   flag it as unconvertible.
2. *ENV-97 ships pre-Stata-6 =.dta= (format 104/105)*, where a =double=
   system-missing is =2^333= = =1.7498005798264095e+100= (see "ENV-97 ships
   at .dta format 104/105" above).  The unit-code column =ga106b= -> =u= is
   *37.2% that sentinel*.  So 1997's unit axis is the wave where a
   reader-level artefact, an undecoded =7.70= (#777) and a currency label all
