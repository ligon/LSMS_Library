...
def _drop_double_punched(flat):
    ...
    that collapse SUMS the additive measures (declared in data_scheme.yml; GH
    #501/#514).  Left in, these rows are silently DOUBLE-COUNTED: 1989 hh 20120
    Fenugreek reports Quantity 4.0 / 1.0 Birr where the source recorded 2.0 /
    0.5.  Across the five roster waves that is 90 rows, 514.9 Quantity and
    343.1 Birr of pure overstatement.
    ...
    30.0 kg) and the declared `sum` correctly folds them together.
