...
- 127 rows (1994a-1997) are a second acquisition of the SAME item, in
  ...
  =_/data_scheme.yml= declares =aggregation: {Quantity: sum, Expenditure:
  sum}= so the collapse to the canonical grain SUMS them; =first= would
  destroy ~50% of the Quantity/Expenditure mass in the affected cells.
...
- 90 rows are byte-identical in the canonical grain AND in both measures
  ...
  0.5).  Because the declared reducer is =sum=, these would be
  DOUBLE-COUNTED (the API reported Fenugreek 4.0/1.0 for that household).
...
The dedup and the =sum= are a MATCHED PAIR.  Neither is correct alone:
drop the declaration and the 127 real rows are destroyed; drop the dedup
and the 90 punch-duplicates are counted twice.
