...
returned nothing for it and it was invisible to every name-keyed
mechanism, including the =mappings:= syntax in =data_info.yml=.  Its two
readers (=_/fct.py=, =2000/_/food_acquired.py=) reached it only via
=df_from_orgfile(name=None)= = "the first table in the file".  Both now
pass =name='harmonize_food'= explicitly, and the =_/Makefile=
prerequisites name =categorical_mapping.org=.

*Do not confuse it with =guatemala.py::harmonized_food_labels=*, which
names =../../_/food_items.csv= -- a DIFFERENT file that does not exist
anywhere in the tree.  That function is dead; GH #783 scoped it out and
left it alone.
