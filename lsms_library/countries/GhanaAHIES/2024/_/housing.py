#!/usr/bin/env python
"""Housing (dwelling characteristics) for GhanaAHIES 2024, all four quarters
(t = 2024Q1 .. 2024Q4) from the household x quarter Sections 5-7 file.

Source: ../Data/2024 AHIES Q1-Q4_SEC01567_edt.dta, Section 7 (asked every
...
Keys and the 332 null-HholdID non-response shells: as food_security.py.
"""
import sys
...
def mapped(s: pd.Series, table: dict, fallback='Other') -> pd.Series:
    ...
    if unmapped:
        print(f'housing 2024: labels not in table -> {fallback!r}: {unmapped}')
...
print(f'housing 2024: dropping {int(shell.sum())} rows with null HholdID (no Section 5-7 content)')
df = df.loc[~shell]
