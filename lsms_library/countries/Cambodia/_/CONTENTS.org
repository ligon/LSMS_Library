...
- Makefile :: FIXED (2026-06-08).  The generic =%.parquet= rule
  ...
  now references =cambodia.py=.  (The =food_items.org= prerequisite this
  bullet mentions is gone: GH #783 retired that file, and Cambodia's
  Makefile never named it in a rule.)
...
- ~~No =categorical_mapping.org=~~ :: *STALE -- this bullet was already
  false when GH #783 read it, and is quoted here rather than deleted so
  the correction is on the record.*  =_/categorical_mapping.org= exists
  (=harmonize_education=, =Roof=, =Floor=) and, since GH #783, also
  carries =harmonize_food=.
- ~~No =food_items.org=~~ :: *STALE, and superseded twice.*  The file was
  created 2026-06-08 (see "DONE food_items.org" below -- the "Known
  issues" bullet was never updated to match), and GH #783 then retired it
  into =categorical_mapping.org=.
...
via =categorical_mapping.org= =harmonize_food= (GH #783; was =food_items.org=), =u= the native unit, =s in {purchased, produced}=.
...
Cambodia's table WAS named (=#+name: food_label=), so unlike GhanaSPS /
Guatemala / Panama it was at least reachable by =df_from_orgfile(name=...)=
-- just not by anything that goes through =categorical_mapping=.

*Idiosyncrasy to preserve*: the =2019-20= column reproduces the source
Stata value labels' encoding artifacts VERBATIM (=sugarâ​ cane/ palm
sugar= contains a zero-width space; =cafÃ©= is mojibake) because that
column is the string join key.  Cleaning it would break the match.
...
ASEANFOODS); see #560.  The =harmonize_food= harmonization + the =FCT ID=
