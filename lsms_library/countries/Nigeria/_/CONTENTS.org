...
*** The C8 SIZE field is recorded by the survey and NOT read by the build

Found 2026-09-08 while comparing =community_prices= against the household
unit value.  The post-harvest community form has *three* price columns, not
two: the W4 questionnaire prints "2. PLEASE USE A QUANTITY OF ONE (1) FOR
[ITEM] AND THE UNIT OF MEASURE YOU ARE USING" over a pair of sub-columns
headed *UNIT OF MEASURE* and *SIZE*, then "3. What is the price for the
item?" in NAIRA (=ph_w4_community_quest.pdf=, C8, p. 13).  The data carry
the SIZE:

| wave   | unit column | size / quantity column(s)             | read by the build? |
|--------+-------------+---------------------------------------+--------------------|
| W1, W2 | *none*      | *none* (unit printed per item on form)  | n/a                |
| W3     | =c8q2= (size baked into the label, e.g. "Heap Small") | =c8q2b= free text ('250g', '75cl', 'wrap') | =c8q2= only, size stripped by =_canon_unit= |
| W4     | =c8q2= ("90. HEAP")  | =c8q2c= coded ("0. SMALL", "34. 250 GRAMS") | *NO* |
| W5     | =c8aq2_b=   | =c8aq2_a= (quantity), =c8aq2_c= (size), =c8aq2_cvn= (kg/l factor) | *NO* |

Two consequences, both measured.

1. *The size axis is discarded.*  =_canon_unit= is documented as returning a
   "size-agnostic base Preferred Label", so "Heap Small" / "Heap Medium" /
   "Heap Large" all become =Heap=, and =community_prices_for_wave= then keeps
   the FIRST of the collapsed duplicates.  W4 raw counts show the axis is
   real and populated: HEAP 2,144 small / 1,986 medium / 409 large; PIECE
   1,450 / 2,595 / 557; MUDU 1,385 small / 817 large; also DERICA, TIYA,
   BOWL, CONGO, WRAP, LOAF, TUBER, BASIN, KOBIOWU.  =food_acquired= keeps
   the size ("small heap", "medium heap", ...), so the two tables sit on
   *different unit axes* and only 15 of =food_acquired='s 81 =u= labels
   intersect =community_prices='s 30.  The 15 that do match include Kg, g,
   cl, l, Bottle/Can, Basin, Basket, Crate, Sack/Bag, Tuber, Wheelbarrow,
   Wrap, Sachet, tin, other specify.

2. *The metric rows are not per-unit prices, which contradicts the
   =data_scheme.yml= contract* ("Price: prevailing price of one unit `u` of
   item `j`").  Where the size field states an explicit metric quantity --
   W4 "30. 20 GRAMS", "34. 250 GRAMS", "36. 500 GRAMS", "24. 75 CL", "23. 60
   CL", "20. 33 CL" -- the reported Naira figure is the price of THAT
   package, stored against =u= = =g= or =cl= as though it were one gram or
   one centilitre.  Measured against the household unit value on matched
   =(t, j, u, geography)= cells (2011Q1-2019Q1, 1,171 matched cells):

   | u  | matched cells | median log gap | median ratio survey/community |
   |----+---------------+----------------+-------------------------------|
   | Kg |           596 |          0.000 | *1.000*                         |
   | l  |           124 |          0.000 | *1.000*                         |
   | g  |           130 |         -5.244 | 0.0053  (~1/189)              |
   | cl |           125 |         -3.738 | 0.0238  (~1/42)               |

   Kg and l -- the units for which "quantity one" is unambiguous -- agree
   *exactly*.  The g and cl ratios are the package sizes the size field
   records (20-500 g; 33-75 cl).  This supersedes the vaguer note in
   =.coder/ledger/591-nigeria-prices.md= section 6 ("community_prices =u='g'= rows
   look mis-scaled ... ratio approximately 1/400"): the cause is now known and it
   is the unread size column, not an unexplained scale error, and =cl= is
   affected as well as =g=.

   *Consumers should exclude =u in (g, cl)= from =community_prices= until this
   is fixed*, or the price is wrong by two orders of magnitude.  Not repaired
   here: restoring the size axis changes the published =u= vocabulary of a
   shipped table and makes it wave-heterogeneous (W1/W2 can never carry it --
   the form prints one unit per item), which is a maintainer design decision,
   not a mapping fix.  The protocol's per-unit remedy (=Price / NumberOfUnits=)
   is available for W4/W5 from the coded size and for W5 from =c8aq2_a= /
   =c8aq2_cvn=; W3's quantity is free text and would need parsing.
