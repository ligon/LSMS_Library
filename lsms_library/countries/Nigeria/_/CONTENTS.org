...
is four columns *for EPAR's purpose*, which is different: EPAR is
=append=-ing the three section files into one long frame in order to BUILD a
crop/unit/condition/size conversion TABLE, so every block's factor is a
legitimate input to that table.  We are not building a table --- we are
carrying the number that sits on the row --- so only the paired column
...
=reported_implausible=.  For context, 171 of the 182 =u='Kg'= rows that carry a
factor report exactly 1.0, and on those 171 rows the survey's factor and the
library's inferred factor agree *perfectly* (=reported_vs_inferred=: 171 both,
0 disagree).
