# Formatting Functions for Nigeria GHS-Panel 2023-24 (W5)
import os
import sys
...
def _folder(wave):
    """Resolve a folder ('2023-24') or PH-quarter ('2024Q1') spelling to the
    wave folder."""
...
def derive_c8_metric_package_unit_price(price, size_quantity):
    ...
    size; where the coded size (c8aq2_c: '24. 75 CL'; the c8aq2_a quantity
    and c8aq2_cvn kg/l factor agree with the code) states a metric quantity
    the reported Naira figure is the price of that package, so the per-unit
...
def inputs_community_prices(wave='2023-24'):
    ...
    quantity, reported price -- one row per W5 C8 record whose coded size
    (c8aq2_c) names a metric quantity (exactly the rows whose served Price
...
def inputs_c8_unrecorded_size_drop(wave):
    """The deletion entry's `inputs` callable: the DROPPED W5 rows.

    One row per W5 C8 record whose unit is metric (g / cl / Kg / l) but
    whose coded size c8aq2_c is blank and not '99. ONE SIZE ONLY': the 133
    rows on '2. GRAMS (G)' -- the only W5 metric-unit rows with no recorded
    size.  (W5's 937 Kg + 504 l rows with a blank size are SERVED under
    the base unit: a blank size beside KILOGRAMS reads as one kilogram; the
    c8aq2_a quantity and c8aq2_cvn factor agree.)  A dropped row has
    nowhere to carry a Derivation key, so the registry entry carries
    `rows: 0` and these are its inputs.
    """
