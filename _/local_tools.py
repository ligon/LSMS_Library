...
def make_parquet_with_rural(df: str, countries:list, year: list = None, rural = True):
    ...
    def rural_for_one_df(country, df, rural, y = None):
        if y == None:
            ...
            fn = f'../{country}/{y}/_/{df}'
        try:
            d = pd.read_parquet(fn + '.parquet')
            r = add_markets_from_other_features(country,d,rural)
            r.to_parquet(fn + '_rural.parquet')
        except FileNotFoundError:
    ...
    dfs = {}
    if year == None:
        # attach to overall dfs for all countries
        for c in countries:
            dfs[c] = rural_for_one_df(c, df, rural)
    else:
        assert len(countries) == len(year), "Lengths of countries and years lists don't match" 
        for c, y in zip(countries, year):
            if type(y) == list:
                # multiple years for one country
                y_dfs = {}
                for sub_y in y:
