...
class TestConfiguration:
    def test_selection_has_exact_disjoint_members(
            self, selected_groups, country_food_table, wave_food_labels):
        declared = set().union(*wave_food_labels.values())
    ...
    def test_country_mapping_covers_every_wave_preferred_label(
            self, country_food_table, wave_food_labels, wave):
        missing = wave_food_labels[wave] - _labels(country_food_table)
        assert not missing, f'{wave}: no country mapping for {sorted(missing)}'
...
class TestAcquired:
    def test_default_retains_preferred_labels(
            self, country, preferred_acquired, wave_food_labels):
        ...
        for wave in WAVES:
            ...
            assert set(fine.index.get_level_values('j')) <= wave_food_labels[wave]
