...
`pyproject.toml`'s static `version` is a `0.0.0` placeholder; the real version
comes from the git tag via poetry-dynamic-versioning, which substitutes the
computed value during a build and restores the placeholder afterwards. A build
killed partway skips that restore -- and one was, on a 4-core Slurm slice. The
substituted value `0.9.1.post208.dev0+fcec3642` was committed and briefly tagged
as v0.10.1 before being caught.

`publish.yml` guards the opposite direction: it fails if the version resolves to
`0.0.0`, meaning the plugin was inactive or tags were not fetched. It cannot
catch this case -- a stale-but-plausible version passes, and PyPI does not allow
a version to be reused. `tests/test_version_placeholder.py` is the missing half.
