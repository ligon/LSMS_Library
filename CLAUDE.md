...
**`catalog_id` is NOT a 1:1 key (GH #600), and a partial record is as dangerous as an absent one.** The relation between WB catalog entries and our wave dirs is many-to-many in *both* directions, so:

- **`#+CATALOG_ID:` is a LIST.** One directory can hold the files of several entries: `Malawi/2016-17/` is `2936, 2939` — `Data/Cross_Sectional/` is IHS4 (2936), `Data/Panel/` is the 2016 wave of the IHPS long-term panel (2939). Repeated `#+CATALOG_ID:` lines **accumulate** (they used to silently last-win). `prov.catalog_id` still returns the *primary* entry, so every pre-#600 caller works unchanged.
- **`#+CATALOG_COVERS:` is a different relation — subsumed, not held.** `Tanzania/2008-15/` holds *only* the Uniform Panel Dataset (3814), whose `round` column carries the four NPS rounds — which is why one directory backs four logical waves (`wave_folder_map`). So it records `#+CATALOG_COVERS: 76, 1050, 2252, 2862`. Listing them under `CATALOG_ID` would claim we hold five entries when we hold one: *the same false claim, pointed the other way.*
- **Relations between catalog *entries*** — one survey catalogued twice in two repositories (`lsms` 297 ≡ `datafirst` 902, South Africa 1993), or an entry *derived from* entries we hold (Nigeria 5835 `NGA_2010-2019_NUPD` = the four GHS-Panel waves, harmonized) — are facts about the **catalog**, not about any directory of ours, and live in `lsms_library/catalog_relations.yml`. Every entry there carries its **evidence**. A `derived` verdict fires only when **every** constituent is held, so it can never over-claim: drop a wave and the entry goes back to being reported missing on its own.
- **`local_status` is a 5-state ladder**: `yes` → `covered` → `derived` → `unknown` → **`no` must be earned**. `covered` / `derived` / `unknown` all keep `local=False` (we do not hold their *files*), so the bool contract is unchanged. `unknown` now also covers a **logical wave inside a multi-round folder** that no record accounts for — so the next Tanzania-shaped folder cannot emit a false "missing wave" even before anyone writes a `covers` line.
- A **re-stamp must not erase what a human established**: `backfill_wave_provenance.py` carries the extra ids, `covers`, the note and the validation through, because the catalog cannot re-derive them.
