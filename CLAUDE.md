...
- The `.mean()` on `Latitude`/`Longitude` is the last aggregation core performs and is **grandfathered, not endorsed** (`_CLUSTER_GPS_COLUMNS`, excluded from the destruction count, counted via `_gps_averaging_stats`). It is a no-op in 4 of the 5 cells where it could fire; in the 5th (Malawi 2013-14) it averages points up to 783 km apart, which is a broken cluster key, not a centroid. Retiring it is recommended — see `.coder/ledger/323-site2.md` §5.
- `i` arriving as a **column** rather than an index level (Uganda's 7 other waves) is Site 2's twin: `drop(columns='i')` launders a household-grain frame into a `(t, v)` frame with unexplained duplicates. Not silent only because Site 1 catches the wreckage (9,890 destroyed + 934 NaN-key).
