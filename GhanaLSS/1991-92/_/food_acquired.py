...
#food expenditure 
with dvc.api.open('../Data/S9B.DTA',mode='rb') as dta:
...
selector_pur.update({f's9bq{i}':f'purchased_value_v{i}' for i in range(2,11)})
...
with dvc.api.open('../Data/S8H.DTA',mode='rb') as dta:
...
selector_pro.update({f's8hq{i}':f'produced_quantity_v{i}' for i in range(3,13)})
