...
*Pre-built parquets*: Pass =use_parquet=True= when constructing a =Country= (or =--use-parquet= on =lsms-library materialize=) to read existing cached parquet files directly.  If a requested parquet is missing, the usual build path runs automatically.
