...
*Bypass DVC on clusters*: Pass =use_parquet=True= when constructing a =Country= (or =--use-parquet= on =lsms-library materialize=) to read the existing =var/*.parquet= files directly. If a requested parquet is missing, the usual DVC/materialize path runs automatically. This is handy on shared filesystems where DVC locks become contentious but the parquet outputs are already present.
...
# Refresh lockfiles (runs dvc repro for every table in Uganda)
poetry run python -m lsms_library.cli refresh-dvc-locks --country Uganda

# Validate without touching the cache (fails if any lock is stale)
poetry run python -m lsms_library.cli refresh-dvc-locks --all --check
...
Use =--dry-run= to preview which =dvc repro= calls would run or add =--force= whenever you need to
rerun stages even if DVC believes they are current. The =--check= mode never mutates the workspace,
making it safe for CI enforcement.
