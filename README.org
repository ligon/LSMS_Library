...
*Cache Location*: Cached files are stored under the user data directory (=~/.local/share/lsms_library/{country}/var/{dataset}.parquet= on Linux, or the platform equivalent).  Override with the =LSMS_DATA_DIR= environment variable.

*Pre-built parquets*: Pass =trust_cache=True= when constructing a =Country= (or =--trust-cache= on =lsms-library materialize=) to read existing cached parquet files directly, skipping all validation.  If a requested parquet is missing, the usual build path runs automatically.

*Controlling Build Backend*: To bypass DVC and build directly with Make (e.g., for debugging):
#+begin_src bash
...
*Managing Cache Size*: Use the CLI to inspect or clear cached files:
#+begin_src bash
...
- *DVC stages* live in =lsms_library/countries/<country>/**/dvc.yaml=. Each stage calls =lsms_library.cli materialize= with =LSMS_BUILD_BACKEND=make= so a fresh dataset is written to =var/*.parquet= before the cache is revalidated. Regenerate these files any time you add a table via =poetry run python -m lsms_library.cli generate-dvc --country <Country>= (or the lower-level =lsms_library.util.generate_dvc_stages= helper).
- *Runtime cache loader* (the =Country.table()= API) consumes those =var/*.parquet= files when =LSMS_BUILD_BACKEND=dvc= (default), and transparently falls back to the Makefile backend if DVC is unavailable or the data scheme requests it.
...
LSMS_BUILD_BACKEND=make lsms-library materialize \
...
- *Contributing*: Contact ligon@berkeley.edu to discuss contributions
